...
//   // Helpers to read display fields safely (handles either .displayName or .name)
//   String _nameOr(VehicleFuelType f) => (f.displayName ?? f.displayName);
//   String _tNameOr(VehicleTransmissionType t) =>
...
//         (m) => m.id == modelId,
//         orElse: () => modelId == null ? _models.first : _models.first,
//       );
//     }
...
//     // If your AddModel.variant holds an id, match by id; if it holds a name, match by name.
...
//         (t) => t.id == txId,
//         orElse: () {
//           final txName = (widget.ad.transmission ?? '').toLowerCase();
//           return _transmissionTypes.firstWhere(
//             (t) => _tNameOr(t).toLowerCase() == txName,
//             orElse: () => _transmissionTypes.isNotEmpty
//                 ? _transmissionTypes.first
//                 : null as VehicleTransmissionType,
//           );
//         },
//       );
//     } catch (_) {}
...
//         (f) => f.id == fuelId,
//         orElse: () {
//           final fName = (widget.ad.fuelType ?? '').toLowerCase();
//           return _fuelTypes.firstWhere(
//             (f) => _nameOr(f).toLowerCase() == fName,
//             orElse: () => _fuelTypes.isNotEmpty
//                 ? _fuelTypes.first
//                 : null as VehicleFuelType,
//           );
//         },
//       );
//     } catch (_) {}
...
//     // Private Vehicle → typically "four_wheeler" and category "private_vehicle"
//     final vehicleType = widget.ad.vehicleType?.isNotEmpty == true
//         ? widget.ad.vehicleType
//         : 'four_wheeler';

//     final category =
//         widget.ad.category.isNotEmpty ? widget.ad.category : 'private_vehicle';
...
//                     .pop(true); // notify detail page to refresh
//               },
//               failure: (msg) {
//                 ScaffoldMessenger.of(context).showSnackBar(
...
//     // You can fetch these from backend too; falling back to a small static list for edit
//     final all = <String>[
//       "Sunroof",
//       "Leather Seats",
//       "Navigation System",
